...
function twoPMakeMove(row, col) {
    if (!gameOver && board[row][col] === '') {
        ...
        if (checkWin()) {
            document.getElementById('twoPlayersWinnerText').textContent =` Player with ${currentPlayer} WINS!`;
            gameOver = true;
        } else if (checkTie()) {
            document.getElementById('twoPlayersWinnerText').textContent =  "IT is  Draw";
            gameOver = true;
        } else {
...
function vsComMakeMove(row, col) {
    if (!gameOver && board[row][col] === '') {
        ...
        if (checkWin()) {
            document.getElementById('vsComputerWinnerText').textContent = ` Player with ${currentPlayer} WINS!`;
            gameOver = true;
        } else if (checkTie()) {
            document.getElementById('vsComputerWinnerText').textContent = "IT is  Draw";
            gameOver = true;
        } else {
            currentPlayer = currentPlayer === 'X' ? 'O' : 'X';
